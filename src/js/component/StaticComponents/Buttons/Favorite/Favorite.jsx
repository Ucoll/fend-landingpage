...
const Favorite = ({ ...props }) => {
  ...
  const [favers, setFavers] = useState([]);
  ...
  useEffect(() => {
    ...
    // ! Tracks the individual favers of the Coll
    //TODO: CurrentUser must be stored or something. Remove HARDCODED USERNAME ("HarryPotter")
    let arr = []
    for (let i = 0; i < props.favs.length; i++) {
      if ("HarryPotter" !== props.favs[i]) {
        arr.push(props.favs[i]);
      }
    }
    setFavers(favers.concat(arr))
  }, []);

  const switchFavorite = () => {
    favorite === "unfavorited"
      ? (setFavorite("favorited"), setFavs(favs + 1))
      : (setFavorite("unfavorited"), setFavs(favs - 1));

    fetch(`${database}/colls/${props.collId}/fav`, {
      method: "POST",
    })
      .then((response) => {
        if (!response.ok) {
  ...
  return (
    <>
      ...
      <span className="coll-favers">
        <div className="favs-hover">{favers.map((fav, index) => {
          while (index < 6){
            return `${fav} `
          }
          return `+${favers.length - 6} people`
          })}</div>
        {`${favs} Favs`}
      </span>
    </>
